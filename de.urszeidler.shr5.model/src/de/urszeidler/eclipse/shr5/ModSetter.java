...
public class ModSetter implements Adapter {
	private Map<EAttribute, Value> fMap = new HashMap<EAttribute, Value>();
	...
	// private static ModSetter instance;
	//
	// public static Object getInstance() {
	// if (instance == null)
	// instance = new ModSetter();
	//
	// return instance;
	// }
	...
	private void addModificationValue(Modifizierbar modifikatoren) {
		...
		for (AttributModifikatorWert wert : mods) {
			if (wert.getAttribut() != null) {
				Value value = fMap.get(wert.getAttribut());
				if (value == null) {
					value = new Value();
					fMap.put(wert.getAttribut(), value);
				}
				value.modWert = value.modWert + wert.getWert();
			}
		}
	}
