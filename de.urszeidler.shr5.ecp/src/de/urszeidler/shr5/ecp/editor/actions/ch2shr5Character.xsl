...
<xsl:stylesheet version="2.0"
    ...
    xmlns:uuid="java.util.UUID" exclude-result-prefixes="uuid shret"
...
<!-- 			<xsl:attribute name="sex"><xsl:value-of select="low sex/text()" /></xsl:attribute>   -->
			<persona  >
			<xsl:attribute name="xsi:type"><xsl:value-of select="shret:findPersonaType(adept/text(),magician/text,technomancer/text())" /></xsl:attribute>
				...
				<xsl:for-each select="cyberwares/*">
					<xsl:value-of disable-output-escaping="yes" select="shret:copyObject(concat(name/text(),' ',rating/text()),id/text(),'koerperMods')"/>
				</xsl:for-each>
...
<!-- 				</xsl:for-each> -->
				...
				<xsl:for-each select="weapons/*">
					<xsl:value-of disable-output-escaping="yes" select="shret:copyObject(name/text(),id/text(),'inventar')"/>
				</xsl:for-each>
				...
				<xsl:for-each select="lifestyles/*">
					<xsl:value-of disable-output-escaping="yes" select="shret:copyObject(name/text(),guid/text(),'contracts')"/>
				</xsl:for-each>
				<xsl:for-each select="vehicles/*">
					<xsl:value-of disable-output-escaping="yes" select="shret:copyObject(name/text(),guid/text(),'vehicels')"/>
				</xsl:for-each>
 ...
 <generatorSrc xsi:type="shr5mngt:FreeStyleGenerator" xmi:id="_B1Q4wN0CEeOvVYoRsPZlnA" character="_B1PDlN0CEeOvVYoRsPZlnA" state="commited" characterName="Ralf">
