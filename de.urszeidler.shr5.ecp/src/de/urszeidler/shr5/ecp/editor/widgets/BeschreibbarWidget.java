...
public class BeschreibbarWidget extends Composite {
	 ...
	 * @param style
	 */
	public BeschreibbarWidget(Composite parent1, int style, Beschreibbar object,
			FormToolkit toolkit) {
		super(parent1, style);
	...
	private void createWidgets(Composite parent1, int style) {
		...
		GridData gd_imageLabel = new GridData(SWT.LEFT, SWT.CENTER, false,
				false, 1, 2);
	...
	protected DataBindingContext initDataBindings() {
		...
		IObservableValue observeValue = EMFObservables.observeValue(
				beschreibbar, Shr5Package.Literals.BESCHREIBBAR__IMAGE);
		...
		// size);
		// } else
		//TODO : also use the editing domain
		converter = new PathToImageConverter(String.class, Image.class);
		UpdateValueStrategy toModel = new UpdateValueStrategy();
		UpdateValueStrategy toWidget = new UpdateValueStrategy()
				.setConverter(converter);
		Binding binding = bindingContext.bindValue(observedImage, observeValue,
				toModel, toWidget);

		//
		IObservableValue observeTextTextNameObserveWidget = WidgetProperties
				.text(SWT.Modify).observe(textName);
		IObservableValue beschreibbarNameObserveValue = EMFObservables
				.observeValue(beschreibbar,
						Shr5Package.Literals.BESCHREIBBAR__NAME);
		bindingContext.bindValue(observeTextTextNameObserveWidget,
				beschreibbarNameObserveValue, null, null);
		//
		IObservableValue observeTextTextBeschreibungObserveWidget = WidgetProperties
				.text(SWT.Modify).observe(textBeschreibung);
		IObservableValue beschreibbarBeschreibungObserveValue = EMFObservables
				.observeValue(beschreibbar,
						Shr5Package.Literals.BESCHREIBBAR__BESCHREIBUNG);
		bindingContext.bindValue(observeTextTextBeschreibungObserveWidget,
				beschreibbarBeschreibungObserveValue, null, null);
		//
