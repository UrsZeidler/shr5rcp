	...
	protected void createFormContent(IManagedForm managedForm) {
		...
		if (object instanceof Koerpermods) {
			...
			if (object instanceof Cyberware) {
				emfFormBuilder.addTextEntry("Einbauten", Shr5Package.Literals.CYBERWARE__EINBAU, grpWert);
			}

		} else if (object instanceof PersonaEigenschaft) {
			grpWert.setText("Karama Kosten");
			emfFormBuilder.addTextEntry("Karma", Shr5Package.Literals.PERSONA_EIGENSCHAFT__KARMA_KOSTEN, grpWert);

		} else if (object instanceof FernkampfwaffeModifikator) {
			grpWert.setText("Erweiterung");
			emfFormBuilder.addTextEntry("Type", Shr5Package.Literals.FERNKAMPFWAFFE_MODIFIKATOR__EP, grpWert);

		}else if (object instanceof KiKraft) {
			grpWert.setText("Kraft Punkte");
			emfFormBuilder.addTextEntry("punkte", Shr5Package.Literals.KI_KRAFT__KRAFTPUNKTE, grpWert);
		}
